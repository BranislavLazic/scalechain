...
import io.scalechain.blockchain.script.{ ScriptSerializer, ScriptValue, TransactionSignature }
import io.scalechain.blockchain.storage.BlockIndex
import io.scalechain.blockchain.storage.index.KeyValueDatabase
import io.scalechain.blockchain.{ ErrorCode, TransactionSignException, UnsupportedFeature }
...
class TransactionSigner()(implicit db: KeyValueDatabase) {
  ...
  /** Sign an input of a transaction with the list of given private keys.
    ...
    */
  protected[transaction] def signInput(
      transaction: Transaction,
      inputIndex: Int,
      normalTxInput: NormalTransactionInput,
    ...
    val scriptData: Array[Byte] = TransactionSignature.getScriptForCheckSig(
      ...
      Array()
    ) // We have no signatures to scrub from the locking script.
  ...
  /** For an input, try to find matching private keys to sign, and sign the input.
    ...
    */
  protected[transaction] def tryToSignInput(
      transaction: Transaction,
      inputIndex: Int,
      privateKeys: List[PrivateKey],
      sigHash: SigHash,
    ...
    inputToSign match {
      case normalTxInput: NormalTransactionInput =>
        val lockingScript: LockingScript =
          new NormalTransactionVerifier(normalTxInput, transaction, inputIndex).getLockingScript(chainView)
        ...
        if (addresses.isDefinedAt(1))
          throw new TransactionSignException(
            ...
            "Multisig is not supported yet. Input Index : " + inputIndex
          )
        if (addresses.isEmpty)
          throw new TransactionSignException(
            ErrorCode.UnsupportedFeature,
            "Unsupported locking script for the transaction input. Input Index : " + inputIndex
          )
        ...
        val keysToUse = privateKeys.filter { key =>
          ...
          if (
            ByteArray(publicKey.getHash().value) == address.publicKeyHash
          ) // If a public key hash matches, we can sign the transaction.
            true
          else
            false
        }
        ...
        if (keysToUse.isEmpty) // We don't have the private key to sign the input.
          ...
          val signedInput: TransactionInput =
            signInput(transaction, inputIndex, normalTxInput, keysToUse, lockingScript, sigHash, chainView)
      ...
      case tx =>
        throw new TransactionSignException(
          ErrorCode.InvalidTransactionInput,
          "An input to sign should be a normal transaction input, not a generation transaction input. Input Index : " + inputIndex
        )
    }
  }
  ...
  /** Signs a transaction from the first input. This is a recursive function with the base case at the end of the inputs.
    ...
    */
  @tailrec
  final protected[transaction] def signInputsFrom(
      transaction: Transaction,
      inputIndex: Int,
      privateKeys: List[PrivateKey],
      sigHash: SigHash,
    ...
    if (inputIndex >= transaction.inputs.length) // The base case. We try to sign all inputs.
      ...
      val newTransaction =
        tryToSignInput(transaction, inputIndex, privateKeys, sigHash, chainView).getOrElse(transaction)
  ...
  /** Merge signatures of the original transaction and newly signed transaction.
    ...
    */
  protected[transaction] def mergeSignatures(
      beforeSigning: Transaction,
      afterSigning: Transaction,
      chainView: BlockchainView
  ): SignedTransaction = {

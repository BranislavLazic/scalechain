...
object BlockVerifier {
  case class TransactionTracker(
      block: Block,
      transaction: Transaction,
      mergedScriptOption: Option[MergedScript],
      count: Int
  )

  val txFailureMapByMessage = mutable.HashMap[String, TransactionTracker]()
  def putTransactionVerificationFailure(
      message: String,
      block: Block,
      transaction: Transaction,
      mergedScriptOption: Option[MergedScript]
    ...
    val (simplestBlock, simplestTransaction, simplestMergedScriptOption) =
      if (tracker.isDefined)
        if (
          transaction.inputs.size + transaction.outputs.size <
    ...
    txFailureMapByMessage(message) =
      TransactionTracker(simplestBlock, simplestTransaction, simplestMergedScriptOption, count + 1)
  }
...
class BlockVerifier(block: Block)(implicit db: KeyValueDatabase) {
  ...
  def verify(chainView: BlockchainView): Unit = {
    ...
    block.transactions.map { transaction =>
      try new TransactionVerifier(transaction).verify(chainView)
      catch {

...
import io.scalechain.util.{ ArrayUtil, ByteArray, ComparableArray, HexUtil }
import ByteArray._
...
object Checksum {
  ...
  val codec: Codec[Checksum] = bytes(VALUE_SIZE).xmap(
    ...
    c => ByteVector(c.value.array)
  ) // internal byte order, should not reverse bytes.
}
...
object Magic {
  ...
  val codec: Codec[Magic] =
    bytes(VALUE_SIZE).xmap(b => Magic.apply(b.reverse.toArray), m => ByteVector(m.value.array.reverse))
}
...
case class BitcoinMessageEnvelope(magic: Magic, command: String, length: Int, checksum: Checksum, payload: BitVector) {

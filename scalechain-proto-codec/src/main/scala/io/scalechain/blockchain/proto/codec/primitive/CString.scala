...
import io.scalechain.blockchain.proto.codec.{ MessagePartCodec, SerializeParseUtil }
...
object CString extends SerializeParseUtil[String] {
  val codec: Codec[String] = filtered(
    ascii,
    new Codec[BitVector] {
...
/** A codec that prefixes data with a null terminated string.
  ...
  */
class CStringPrefixedCodec[T <: ProtocolMessage](codecT: MessagePartCodec[T])
    extends MessagePartCodec[CStringPrefixed[T]] {

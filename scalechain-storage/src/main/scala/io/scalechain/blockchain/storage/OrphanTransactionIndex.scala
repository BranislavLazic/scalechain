...
trait OrphanTransactionIndex {
  ...
  /** Put an orphan transaction
    ...
    */
  def putOrphanTransaction(hash: Hash, orphanTransactionDescriptor: OrphanTransactionDescriptor)(implicit
      db: KeyValueDatabase
  ): Unit =
  ...
  /** Add an orphan transaction than depends on a transaction denoted by the hash.
    ...
    */
  def addOrphanTransactionByParent(missingTransactionHash: Hash, orphanTransactionHash: Hash)(implicit
      db: KeyValueDatabase
  ): Unit =
    // TODO : Optimize : Reduce the length of the prefix string by using base64 encoding?
    db.putPrefixedObject(
      ORPHAN_TRANSACTIONS_BY_DEPENDENCY,
      hex(missingTransactionHash.value),
      orphanTransactionHash,
      OneByte(1)
    )
  ...
  def getOrphanTransactionsByParent(missingTransactionHash: Hash)(implicit db: KeyValueDatabase): List[Hash] =
    (
      using(
        db.seekPrefixedObject(ORPHAN_TRANSACTIONS_BY_DEPENDENCY, hex(missingTransactionHash.value))(
          HashCodec,
          OneByteCodec
        )
      ) in {

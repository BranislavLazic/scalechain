...
import io.scalechain.blockchain.script.{ BlockPrinterSetter, ScriptBytes, ScriptParser }
...
object DumpChain {
  ...
  def mergeScripts(blocksPath: String, filterOption: String): Unit = {
    ...
    def p2pkh_filter(pkPrefix: Byte): ScriptFilter = { (unlockingScript, lockingScript) =>
    ...
    /**
      ...
      * @return
      */
    def locking_script_has(opCode: OpCode): ScriptFilter = { (unlockingScript, lockingScript) =>
    ...
    def p2sh_filter(): ScriptFilter = { (unlockingScript, lockingScript) =>
      val scriptOps = ScriptParser.parse(lockingScript)
              ...
              case normalTxIn: NormalTransactionInput =>
                ...
                if (txWithOutputOption.isDefined) { // Found the transaction object from the map.
                  ...
                  if (filter(unlockingScript, lockingScript)) {
                    println(
                      s"MergedScript(transaction=$tx, inputIndex=$inputIndex, unlockingScript=${unlockingScript}, lockingScript=${lockingScript})"
                    )
                    println()
                  }
                }

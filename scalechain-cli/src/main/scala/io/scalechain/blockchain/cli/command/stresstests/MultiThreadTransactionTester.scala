...
import io.scalechain.blockchain.cli.command.stresstests.MultiThreadTransactionTester.{
  RawTransactionWithGroupListener,
  ThreadGroupIndexFilter,
  TransactionWithGroupListener
}
...
class MultiThreadTransactionTester(threadGroupIndexFilter: Option[ThreadGroupIndexFilter] = None) {
  ...
  /**
    ...
    */
  def testTransaction(
      initialSplitTxTest: TransactionWithGroupListener,
      transactionTests: IndexedSeq[TransactionWithGroupListener]
  ): Unit = {
  ...
  def testRawTransaction(
      initialSplitTxTestWithGroup: RawTransactionWithGroupListener,
      transactionTests: IndexedSeq[RawTransactionWithGroupListener]
  ): Unit = {
    // Step 2 : Load each transaction group file, run tests in parallel for each group.
    val threads =
      (0 until transactionTests.length)
        .filter { i =>
        ...
        .map { i =>
          new Thread() {
            override def run(): Unit = {
              ...
              val initialTxReader =
                new TransactionReader(new File(GenerateRawTransactions.initialSplitTransactionFileName()))

...
import io.scalechain.blockchain.proto.{ Hash, OutPoint, Transaction, TransactionOutput }
...
class TransactionGenerator(wallet: Wallet)(implicit db: KeyValueDatabase) extends AutoCloseable {
  ...
  def newTransaction(
      ...
      inputStartIndex: Int,
      inputEndIndex: Int,
      newOwnerAddresses: IndexedSeq[CoinAddress]
  ): Transaction = {
  ...
  def signTransaction(transaction: Transaction, privateKeysOption: Option[List[PrivateKey]] = None): Transaction = {
    val signedTransaction: SignedTransaction =
      Wallet.get.signTransaction(transaction, chainView, List(), privateKeysOption, SigHash.ALL)

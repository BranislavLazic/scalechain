...
import io.scalechain.blockchain.cli.command.{ Command, RpcInvoker, RpcParameters }
...
//        println(s"sendThreadTransaction - Using port ${port}, txGroupIndex=${txGroupIndex}, nodeCount=${nodeCount}")
    ...
    val threadGroupIndexFilter =
      nodeFilterIndexOption.map { nodeFilterIndex => (txGroupIndex: Int) =>
        (txGroupIndex % nodeCount) == nodeFilterIndex
      }

    val startTimeMillis = System.currentTimeMillis()
    new MultiThreadTransactionTester(threadGroupIndexFilter)
      .testRawTransaction(sendSplitTransaction, IndexedSeq.fill(transactionGroupCount)(sendThreadTransaction))

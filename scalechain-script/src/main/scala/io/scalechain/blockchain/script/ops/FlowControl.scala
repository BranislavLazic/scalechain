...
import io.scalechain.blockchain.script.{ ParseResult, ScriptEnvironment, ScriptOpList, ScriptParser }
...
trait IfOrNotIfOp extends FlowControl {
  def create(script: Script, offset: Int, invert: Boolean): (ScriptOp, Int) = {
    ...
    (OpCond(invert, thenPart.scriptOpList, elseScriptOpList), thenPart.bytesConsumed + elsePartBytesConsumed)
  }
}

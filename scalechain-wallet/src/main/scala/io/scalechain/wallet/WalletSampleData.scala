...
import io.scalechain.blockchain.chain.{ ChainEventListener, ChainSampleData }
import io.scalechain.blockchain.storage.index.KeyValueDatabase
import io.scalechain.blockchain.transaction.{ CoinAddress, TransactionTestDataTrait }

class WalletSampleData(wallet: Wallet)(protected override val db: KeyValueDatabase)
    extends ChainSampleData(Some(wallet))(db) {

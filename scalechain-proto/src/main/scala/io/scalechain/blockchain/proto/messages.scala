...
case class Version(
  ...
  override def toString =
    s"""Version($version, ${bint(services)}, ${timestamp}L, $destAddress, $sourceAddress, ${bint(
      nonce
...
/** GetBlocks; Return an inv packet containing the list of blocks
  ...
  */
case class GetBlocks(version: Long, blockLocatorHashes: List[Hash], hashStop: Hash) extends ProtocolMessage {
...
/** GetHeaders ; Return a headers packet containing the headers of blocks starting right after the last known hash
  ...
  * if the block locator object contains a hash on the invalid branch.
  */
case class GetHeaders(version: Long, blockLocatorHashes: List[Hash], hashStop: Hash) extends ProtocolMessage {
...
object RejectType extends Enumeration {
  ...
  val REJECT_MALFORMED, REJECT_INVALID, REJECT_OBSOLETE, REJECT_DUPLICATE, REJECT_NONSTANDARD, REJECT_DUST,
      REJECT_INSUFFICIENTFEE, REJECT_CHECKPOINT = Value
}
import RejectType._

/** Reject ; The reject message is sent when messages are rejected.
  */
case class Reject(message: String, rejectType: RejectType, reason: String, data: ByteArray) extends ProtocolMessage {

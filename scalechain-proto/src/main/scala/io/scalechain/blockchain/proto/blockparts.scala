...
import io.scalechain.util.{ BigIntUtil, ByteArray, HexUtil, Utils }
...
case class NormalTransactionInput(
    override val outputTransactionHash: Hash,
...
case class GenerationTransactionInput(
    override val outputTransactionHash: Hash,
...
/** The block message is sent in response to a getdata message
  ...
  */
case class Block(val header: BlockHeader, val transactions: List[Transaction]) extends ProtocolMessage {

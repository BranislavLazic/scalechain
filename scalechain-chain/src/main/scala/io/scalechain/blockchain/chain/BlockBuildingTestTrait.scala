...
trait BlockBuildingTestTrait extends TransactionTestDataTrait {
  ...
  /** Create a generation transaction
    ...
    */
  def generationTransaction(name: String, amount: CoinAmount, generatedBy: OutputOwnership): TransactionWithName = {
    val transaction = TransactionBuilder
      .newBuilder()
      // Need to put a random number so that we have different transaction id for the generation transaction.
      .addGenerationInput(
        CoinbaseData(
          s"Random:${Random.nextLong}.The scalable crypto-currency, ScaleChain by Kwanho, Chanwoo, Kangmo.".getBytes
        )
      )
  ...
  def normalTransaction(
      name: String,
      spendingOutputs: List[OutputWithOutPoint],
      newOutputs: List[NewOutput]
  ): TransactionWithName = {
  ...
  def mineBlock(chain: Blockchain)(implicit db: KeyValueDatabase) = {
    ...
    val COINBASE_MESSAGE = CoinbaseData(
      s"height:${chain.getBestBlockHeight() + 1}, ScaleChain by Kwanho, Chanwoo, Kangmo.".getBytes
    )

...
import io.scalechain.blockchain.proto.{ Hash, Transaction, TransactionPoolEntry }
...
class TransactionPool(val storage: BlockStorage, txMagnet: TransactionMagnet) {
  ...
  def getOldestTransactions(count: Int)(implicit db: KeyValueDatabase): List[(Hash, Transaction)] =
    storage
      .getOldestTransactionHashes(count)
      .map {
        case key @ CStringPrefixed(createdAtString, txHash) =>

...
object RpcError {
  ...
  val RPC_FORBIDDEN_BY_SAFE_MODE =
    ErrorDescription(
      -2,
      "RPC Forbidden By Safe Mode"
    ) // Unable to run a command that is not allowed in safe mode when the server is in safe mode.
  ...
  val RPC_DESERIALIZATION_ERROR =
    ErrorDescription(-22, "RPC Deserialization Error") // Error parsing or validating structure in raw format.
  // fundrawtransaction : "TX decode failed"

  val RPC_VERIFY_ERROR =
    ErrorDescription(-25, "RPC Verify Error") // General error during transaction or block submission.
  val RPC_VERIFY_REJECTED =
    ErrorDescription(-26, "RPC Verify Rejected") // Transaction or block was rejected by network rules.
  ...
  val RPC_CLIENT_IN_INITIAL_DOWNLOAD =
    ErrorDescription(-10, "RPC Client In Initial Download") // Still downloading initial blocks
  ...
  val RPC_CLIENT_NODE_NOT_CONNECTED =
    ErrorDescription(-29, "RPC Client Node Not Connected") // Node to disconnect not found in connected nodes.
  val RPC_CLIENT_INVALID_IP_OR_SUBNET = ErrorDescription(-30, "RPC Client Invalid IP or Subnet") // Invalid IP/Subnet

  // Wallet errors
  val RPC_WALLET_ERROR =
    ErrorDescription(-4, "RPC Wallet Error") // Unspecified problem with wallet ( key not found, etc.)
  ...
  val RPC_WALLET_INSUFFICIENT_FUNDS =
    ErrorDescription(-6, "RPC Wallet Insufficient Funds") // Not enough funds in wallet or account.
  ...
  val RPC_WALLET_KEYPOOL_RAN_OUT =
    ErrorDescription(-12, "RPC Wallet Keypool Ran Out") // Keypool ran out, call keypoolrefill first.
  ...
  val RPC_WALLET_UNLOCK_NEEDED =
    ErrorDescription(-13, "RPC Wallet Unlock Needed") // Enter the wallet passphrase with walletpassphrase first.
  // EnsureWalletIsUnlocked : "Error: Please enter the wallet passphrase with walletpassphrase first."

  val RPC_WALLET_PASSPHRASE_INCORRECT =
    ErrorDescription(-14, "RPC Wallet Passphrase Incorrect") // The wallet passphrase entered was incorrect.
  ...
  val RPC_WALLET_WRONG_ENC_STATE =
    ErrorDescription(
      -15,
  ...
  val RPC_WALLET_ENCRYPTION_FAILED =
    ErrorDescription(-16, "RPC Wallet Encryption Failed") // Failed to encrypt the wallet.

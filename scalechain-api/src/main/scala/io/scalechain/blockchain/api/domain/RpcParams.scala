    ...
    if (minValueOption.isDefined)
      if (paramValue < minValueOption.get)
        throw new RpcException(
          ErrorCode.RpcArgumentLessThanMinValue,
          s"A parameter $paramName at index $paramIndex is less than the minimum value ${minValueOption.get}."
        )
    if (maxValueOption.isDefined)
      if (paramValue > maxValueOption.get)
        throw new RpcException(
          ErrorCode.RpcArgumentGreaterThanMaxValue,
          s"A parameter $paramName at index $paramIndex is greater than the minimum value ${maxValueOption.get}."
        )
  }
}

case class LongRangeValidator(minValueOption: Option[Long], maxValueOption: Option[Long])
    extends ParameterValidator[Long] {
...
case class BigDecimalRangeValidator(
    minValueOption: Option[scala.math.BigDecimal],
    maxValueOption: Option[scala.math.BigDecimal]
) extends ParameterValidator[scala.math.BigDecimal] {
...
case class RpcParams(paramValues: List[JsValue]) {
  ...
  def getOption[T](name: String, index: Int, parameterValidators: List[ParameterValidator[T]] = List())(implicit
  ...
  def get[T](name: String, index: Int, parameterValidators: List[ParameterValidator[T]] = List())(implicit
      reader: spray.json.JsonReader[T]
  ): T = {
    val valueOption = getOption[T](name, index, parameterValidators)
    valueOption.getOrElse {
      throw new RpcException(
        ErrorCode.RpcMissingRequiredParameter,
        s"A mandatory parameter $name at index $index is missing."
      )
    }
  }

...
import io.scalechain.blockchain.{ ErrorCode, ExceptionWithErrorCode, RpcException }
    ...
    try block
      ...
      case e: ExceptionWithErrorCode =>
        if (e.isInstanceOf[RpcException]) {
          ...
          logger.error(
            s"ExceptionWithErrorCode, while executing a command. exception : $e, message : ${e.message}, occurred at : ${StackUtil
              .getStackTrace(e)}"
          )
        RpcCommand.ERROR_MAP.get(e.code) match {
          ...
          case _ =>
            ...
            logger.error(
              s"No mapping to RPC error for exception : $e, message : ${e.message}, occurred at : ${StackUtil.getStackTrace(e)}"
            )
            Left(RpcError(rpcError.code, rpcError.messagePrefix, e.getMessage))
        }
      ...
      case e: Exception =>
        logger.error(
          s"Internal Error, while executing a command. exception : $e, message : ${e.getMessage}, occurred at : ${StackUtil
            .getStackTrace(e)}"
        )
        val rpcError = RpcError.RPC_INTERNAL_ERROR
        Left(RpcError(rpcError.code, rpcError.messagePrefix, e.getMessage))

...
object TransactionFormatter {
  ...
  def getRawTransaction(
      ...
      blockInfoOption: Option[BlockInfo]
  ): RawTransaction = {
    ...
    val confirmations =
      if (blockInfoOption.isDefined)
        if (bestBlockHeight >= blockInfoOption.get.height)
          ...
          logger.error(
            s"The best block height(${bestBlockHeight}) is less than the block height${blockInfoOption.get.height} which has a transaction."
          )
          assert(false)
          0L
        }

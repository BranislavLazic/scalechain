...
import io.scalechain.blockchain.net.{ NodeClient, Peer, PeerSet }
            ...
            .addListener(new ChannelFutureListener() {
              def operationComplete(future: ChannelFuture) {
                ...
                if (future.cause() != null) { // completed with failure
                  ...
                  logger.warn(s"Failed to close connection. Remote address : ${channel
                    .remoteAddress()}. Exception : ${future.cause.getMessage}, Stack Trace : ${StackUtil
                    .getStackTrace(future.cause())} ${causeDescription}")
                }

...
import io.scalechain.blockchain.proto.{ GetBlocks, GetData, ProtocolMessage }
...
object GetBlocksMessageHandler {
  ...
  def handle(context: MessageHandlerContext, getBlocks: GetBlocks): Unit = {
    ...
    val blockHashes = locator.getHashes(
      BlockLocatorHashes(getBlocks.blockLocatorHashes),
      getBlocks.hashStop,
      maxHashCount = MAX_HASH_PER_REQUEST
    )
